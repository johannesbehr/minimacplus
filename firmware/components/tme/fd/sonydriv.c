...
static void load_track_data(device_t *device,int floppy_select)
{
	...
	//cur_image = floppy_get_device_by_type(device_machine, FLOPPY_TYPE_SONY, floppy_select);
	//cur_image = floppy_get_device_by_type(device_machine, FLOPPY_TYPE_SONY, floppy_select);
...
*/
	...
	/*
	if (f->loadedtrack_data) auto_free(device_machine,f->loadedtrack_data);
	new_data = auto_alloc_array(device_machine,UINT8,track_size);
	if (!new_data)
	{
		return;
	}
	*/
	...
	floppy_drive_read_track_data_info_buffer(f->head, new_data, &track_size);
...
UINT8 sony_read_data(device_t *device)
{
	...
	cur_image = floppy_get_device_by_type(device_machine, FLOPPY_TYPE_SONY, sony.floppy_select);
	if (!cur_image->exists())
		return 0xFF;
...
void sony_write_data(device_t *device,UINT8 data)
{
	...
	f = &sony.floppy[sony.floppy_select];
	cur_image = dynamic_cast<device_image_interface *>(floppy_get_device_by_type(device_machine, FLOPPY_TYPE_SONY, sony.floppy_select));
	if (!cur_image->exists())
		return;
...
int sony_read_status(device_t *device)
{
	...
	if (LOG_SONY_EXTRA)
	{
		printf("sony.status(): action=%x pc=0x%08x%s\n",
			action, (int) device_machine.firstcpu->pc(), sony.floppy_enable ? "" : " (no drive enabled)");
	}
	...
	if ((! sony_enable2()) && sony.floppy_enable)
	{
		...
		case 0x0e:  /* Tachometer */
			...
			if (cur_image)
			{
				result = ((int) (device_machine.time().as_double() / 60.0 * sony_rpm(f, cur_image) * 60.0 * 2.0)) & 1;
			}
			break;
...
static void sony_doaction(device_t *device)
{
	...
	if (LOG_SONY)
	{
		logerror("%s sony_doaction(): action=%d %s\n",
			device_machine.describe_context(), action, (sony.floppy_enable) ? "" : " (MOTOR OFF)");
	}
...
void sonydriv_floppy_image_device_device_start()
{
